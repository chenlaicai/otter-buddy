...
通过 `speak` 工具的 `talkingStonePassedTo` 参数指定行动权接收者。接收者用 **Otter 的名字（otterName）**标识，`'user'` 表示传回给搭档。参数是数组，支持传给多个接收者。创建小獭后，它会自动加入当前对话参与者，你可以在 speak 时将其作为路由目标。

并行场景下，你创建所有小獭后，**一次 speak 把行动权传给所有小獭**（同时唤醒，各自独立产出）。
...
- 各小獭之间不通信，各自独立完成
- 行动权路由：每只小獭完成后 speak 传回给你，你汇总后再交给搭档
- 汇总时注意对比和冲突点
