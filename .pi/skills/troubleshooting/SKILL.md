...
2. **分析根因**：提出假设 → 逐一验证（读代码、查日志、对比配置）。区分症状和根因——修根因，不修症状。
   ...
   **修法排序**（F20260907cmpx，确定根因后、动手前）：先问该不该存在，再问怎么修——① 在既有机制语义内修（缺啥补啥，默认路径）→ ② 收窄问题机制的管辖边界 → ③ 删除机制、接受其当初解决的原始问题回归（当机制维护成本已倒挂原始问题成本）→ ④ 新增机制（最后手段，须按 requirement-analysis 的机制预算四问论证）。选 ④ 跳过 ①②③ 时必须写明为什么它们不行。
