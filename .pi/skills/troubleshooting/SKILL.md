...
1. **预注册**（动手前三行，防 HARKing）：看到数据之前，先冻结预期——
   - 我预期根因方向是 X（一句话，可粗）
   - 验证标准是 Y（什么证据出现 = 预期成立）
   ...
   **预期须可证伪**——X/Y/Z 三行必须能被真实证据驳斥；写「根因在代码里」「找到错误代码」这类永远正确的模糊预期等同未预注册。
   ...
   预注册写入排查结论的开头（特性文档/汇报均带），事后可对照「预期 vs 实际」。依据：Anthropic AAR mini-paper 预注册——看到结果后挑一个最好看的故事（HARKing）比先入为主更隐蔽；「先收集数据再归纳」管住了先推测后找数据，本条管住另一半。成本近乎零，写不出三行说明问题定义还太模糊——先澄清再动手。
2. **收集信息**：读取相关文件、日志、配置；查询 memory 中的历史决策和类似问题；确认复现条件和影响范围。
3. **分析根因**：提出假设 → 逐一验证（读代码、查日志、对比配置）。区分症状和根因——修根因，不修症状。

   **修法决策树**（原称「修法排序」，确定根因后、动手前）：先问该不该存在，再问怎么修——① 在既有机制语义内修（缺啥补啥，默认路径）→ ② 收窄问题机制的管辖边界 → ③ 删除机制、接受其当初解决的原始问题回归（当机制维护成本已倒挂原始问题成本）→ ④ 新增机制（最后手段，须按 requirement-analysis 的机制预算四问论证）。选 ④ 跳过 ①②③ 时必须写明为什么它们不行。

   **机制识别检查点**（修法决策树选定前逐项打勾，机械判断不凭感觉）：本次修复是否涉及以下任一项？
   - □ 新增配置字段/枚举/开关
   - □ 新增状态生命周期（创建→存活→销毁）
   - □ 新增定时任务/后台进程
   - □ 新增信号类型/消息格式
   ...
   命中任何一项 → 修复大概率含净新增机制。后续约束：若最终走修法决策树④ → 四问必答（作者当场作答，写入特性文档）+ 重对抗门必过；若排查者判断走①②③ → 必须在排查结论中显式论证「为何命中清单但不涉净新增机制」（一句话即可），论证写入特性文档供对抗审视核验——论证不出 → 改走④流程。全部未命中 → 默认走修法决策树①②③。清单故意偏严——误触发成本是写几行字，漏掉机制的成本是补丁叠加。
