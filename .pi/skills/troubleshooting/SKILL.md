...
3. **分析根因**：提出假设 → 逐一验证（读代码、查日志、对比配置）。区分症状和根因——修根因，不修症状。
   ...
   **机制识别检查点**（修法排序前逐项打勾，机械判断不凭感觉）：本次修复是否涉及以下任一项？
   ...
   命中任何一项 → 修复大概率含净新增机制。后续约束：若最终走修法排序④ → 四问必答（作者当场作答，写入特性文档）+ 重对抗门必过；若排查者判断走①②③ → 必须在排查结论中显式论证「为何命中清单但不涉净新增机制」（一句话即可），论证写入特性文档供对抗审视核验——论证不出 → 改走④流程。全部未命中 → 默认走修法排序①②③。清单故意偏严——误触发成本是写几行字，漏掉机制的成本是补丁叠加。
