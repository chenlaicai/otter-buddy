...
2. **分析根因**：提出假设 → 逐一验证（读代码、查日志、对比配置）。区分症状和根因——修根因，不修症状。
   ...
   **存在性对抗分级**（修法排序选定后，F20260908pgrd）：
   - 选 ④（新增机制）→ **重对抗门**：动手前召检视獭（轮换、异模型）审查「这个修改在系统演化史中是治本还是治标」（要求检视獭 search_memory 查相关机制演化脉络）。门控三结论：`确认治本` / `疑似治标（附理由）` / `信息不足（附缺口说明）`——仅持「确认治本」方可继续；「疑似治标」暂停呈搭档；「信息不足」作为合法结论呈搭档，禁止硬给质量可疑的答案
