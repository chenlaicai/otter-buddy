...
`scripts/lint-skills.mjs` 的校验项（F20260811sktp）。共 7 项，按严重程度分 error（阻断）和 warning（不阻断）。
...
| 3 | manifest.yaml 中每个 skill 与 SKILL.md frontmatter 一致（name 相同；目录集合一致） | manifest 有 `xxx` 但 `.pi/skills/xxx/` 不存在 |
| 4 | manifest 中 `next` 指向的 skill 必须存在 | next: [non-existent] |
| 5 | manifest 中 `not_for` 提到的 skill 必须存在 | not_for 写了不存在的 skill 名 |
| 6 | manifest 与 `.pi/skills/` 集合一致（无孤立） | SKILL.md 存在但 manifest 缺；或 manifest 有但 SKILL.md 缺 |
