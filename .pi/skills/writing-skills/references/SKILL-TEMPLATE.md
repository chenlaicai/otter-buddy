...
- **格式**：参考 worktree 中的 `docs/features/` 下已有文档的 frontmatter 格式。核心字段：`id`、`title`（人类可读描述，不用英文 slug——slug 放文件名，#470）、`summary`、`change_type` + `capability_test`（change_type 为 feature/prompt 时声明，指向 `tests/capability/` 用例或 `n/a: 理由`）+ `created_in_conversation`。可选：`doc_type`、`causal_links`、`tags`、`modules`、`created_at`（使用率低，按需选用）。**新文档不再写 `status` 字段**（F20260902sigm：特性链才是系统来时路，单篇文档的手工状态字段必然腐烂；存量文档的 status 保持原地不动、无人消费）
