...
5b. **教训段验收条**（EPD 对照）：skill/prompt/SYSTEM.md 中沉淀「#xxx 教训/现场」类段落时，每条教训必须含「不这么做的现场」三要素——**①当时的错误现象 ②导致的后果 ③定位过程**；只有结论没有现场的教训是半成品（「复制结论 ≠ 复制产生它的推理」——字节 EPD 实测：直接 SFT 只保住 3.8% 收益，拟合单步决策保住 64.8%），审视时打回。范本：code-implementation 的「废弃资源清理（教训）」段（错查孤儿库→「零事件」假象→错误结论险胜搭档记忆）。存量教训段不要求回改，新增/修订教训段必须达标。

   **编号不进教训段正文**：三要素是现象/后果/定位——**不含编号要求**。F 编号与 issue 号是出处不是现场，出处归特性文档 frontmatter causal_links 与 git 历史（pre-commit lint-prompt-anchors 机械拦截注入面锚点）。写教训段时克制「附编号证明有出处」的反射——A1 证据锚点管对话层断言，不管文件写入。

   **判定示例**（模糊地带的过/不过参照）：
   - ✅ 过（三要素齐）：「错查孤儿库 otter.db（现象）→ 得「零事件」假象误导核查（后果）→ 残留 6 天才被搭档抓包（定位）」
   - ❌ 不过（仅结论）：「教训：建 issue 前必须过关联度前置闸」——无现象无后果无定位
   - ❌ 不过（流水账 ≠ 现场）：「那天改了三版才过」——有过程叙述但无错误现象与后果，读者仍不知为何存在此规则
   - 多行段落：现场要素可分布在同段后续行，但首行结论后 3 行内必须出现，读者不应翻段找现场

   **触达说明**：本条虽是 writing-skills 的子节，但管辖面是「skill/prompt/SYSTEM.md 一切教训段的写入」——写 SYSTEM.md 或 prompts/scheduled/*.md 教训段的獭不经过本 skill，执行靠审视侧核查项（adversarial-review review-dimensions.md 教训段三要素核查）兜底；本条是写法真相源，审视条是执行闸。
