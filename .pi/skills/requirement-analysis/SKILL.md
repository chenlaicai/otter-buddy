...
6. **产出方案**：将技术方案写入 worktree 中的特性文档（参见全局约定「特性文档」）。首次写入时用 `create_linked_resource(type: "file", groupId: "<特性ID>")` 注册（groupId 可选），后续参与者通过 `list_artifacts` 发现。写完后调 `sync_docs`（root_dir 传 worktree 绝对路径）立即入库，并用 `link_memory` 声明"当前讨论 produced 本文档"——让"这文档怎么来的"之后可被 get_related 拼出链。按下方模板组织内容。
   ...
   **机制预算四问**（方案净新增机制/子机制/对外承诺时必答，答案写入「设计取舍」段；①②答不出 → 不做，④答不出 → 标注「永久承诺」呈搭档确认）：
   ...
   **重对抗门**（方案净新增机制时，F20260908pgrd）：四问答毕、方案落盘前，召检视獭（轮换、异模型）审查「这个机制在系统演化史中是治本还是治标」（四问答案是审查材料）。门控三结论：`确认治本` / `疑似治标（附理由）` / `信息不足（附缺口说明）`——仅持「确认治本」方案方可定稿；「疑似治标」呈搭档裁决；「信息不足」作为合法结论呈搭档（禁止硬答），搭档裁决：补充信息后重审 / 基于已有信息自行决定继续或中止。门控结论留痕「设计取舍」段。重对抗审查与步骤 7 对抗审视可合并为同一只检视獭的同一轮审视（审查问题中追加门控三问），不重复召唤。
