...
4. **推送并创建 PR**：`git push -u origin <branch>` + `gh pr create`，PR 链接交给搭档。本期创建的 feature 分支 rebase 后需重写推送时，用 `git push --force-with-lease`（#468：R1 红线对这类分支放行 force-with-lease；受保护分支的 force push 仍禁止）。
