...
description: >-
  Use when: 对抗审视、delta 复核、审视报告留痕场景——PR 创建或方案落盘后组织独立审视时.
  Not for: 审视报告本身的写法与分级标准 → adversarial-review. 獭间意见冲突裁决 → conflict-resolution-protocol.
  Output: 完整审视循环（召唤检视獭 → 合规校验 → 逐条处置 → delta 复审 → 收敛/裁决/终审）.
  能力摘要：代码 PR 审视与方案审视两套协议的编排查表.
co_loads: []
category: reference
---
...
PR 创建（或方案落盘）后，交付不算完成——必须经独立审视。本 skill 定义审视循环的编排：召唤、处置、复审、收敛、终审。

## 触发

**触发条件**：PR 创建后或方案落盘后，组织独立对抗审视、组织 delta 复审、审视报告留痕时。

**排除**：审视报告本身的结构与发现分级标准 → `adversarial-review`（本协议要求检视獭先读它）。作者逐条处置的决策树细则 → `../adversarial-review/references/author-response-protocol.md`。

**输入**：
| 输入 | 必选 | 缺失时 |
|------|------|--------|
| 审视对象（PR 号或方案文档） | 是 | 停下来问搭档 |
| worktree 绝对路径（代码 PR） | 否 | 无 worktree 场景跳过 |
...
3. **复审循环**：按结论修订方案并复审。第 2 轮起是 **delta 审视**：把上轮发现清单 + 你的逐条处置（含更好/更差判断）+ 修订 diff + **更新后的方案文档**（delta 审视需核对建议发现是否写入待办/决策史节，标注段落定位）发给检视獭（轮次结构与检视者职责定义见 `../adversarial-review/references/review-loop.md`）。复审按收敛判据运转：不设轮数上限，自然终止于"修复验证全部通过 + 无严重发现未处置 + 无阻断回归"；对立僵局 / 移动靶 / 僵尸循环任一信号 → 呈搭档裁决。
4. **决策史回写**：决策史回写文档——每道题的结论和理由留痕。

以上走完，方案才算定稿，才可进入实现阶段。

**通用约束**：审视者必须独立于实现者——自己写自己审等于没审。搭档明确表示"跳过审视/不用审"时，记录该决策后放行。
