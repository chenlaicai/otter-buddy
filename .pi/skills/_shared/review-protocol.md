...
修复后更新 PR，重新走审视（systemPrompt 不可更新：在消息中把新 diff 发给检视獭，或 dissolve 后重建）。第 2 轮起是 **delta 审视**——重建材料：上述全部材料 + 上轮发现清单 + 你的逐条处置 + 修复 diff（轮次结构与检视者职责定义见 `adversarial-review/references/review-loop.md`）
...
按结论修订方案并复审。第 2 轮起是 **delta 审视**：把上轮发现清单 + 你的逐条处置 + 修订 diff 发给检视獭（轮次结构与检视者职责定义见 `adversarial-review/references/review-loop.md`）。复审按收敛判据运转：不设轮数上限，自然终止于"修复验证全部通过 + 无阻断回归"；对立僵局 / 移动靶 / 僵尸循环任一信号 → 呈搭档裁决。
