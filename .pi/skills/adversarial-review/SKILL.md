...
2. **声明本轮焦点**：检查任何内容之前，先声明 1-3 个焦点维度及理由。什么出错后果最严重？那就是焦点。焦点维度 → 深入读周边代码、追踪执行路径、验证声明。非焦点维度 → 快速扫过。无焦点的审视是散弹枪审视。
   ...
   > **作者「检视焦点建议」的防锚定规则（事故教训）**：作者给出的检视焦点建议——**无论载体**（PR 描述/召唤 systemPrompt 的「重点对抗方向」/口头叮嘱）——只能作为输入之一，**不能替代你自己的焦点声明**。作者的盲点会原样写进焦点建议（现场：4 条焦点建议全是 ID 语义，迁移机制 CTAS 不在任何一条里，检视注意力被锚定导致漏检）。**机械化顺序**：①先不看作者清单，独立判断「什么出错后果最严重」写下自己的焦点声明 → ②再读作者清单，核对是否有你没覆盖的项 → ③报告中显式披露比对结果与你是否感到被锚定（正向反向都算——先读清单让你倾向找它的反例也是锚定）。对「变更机制本身」（如何实现的，而非实现是否符合语义）永远保留一个独立检查动作，无论作者焦点是否提及。
   >
