...
**Fix**: Every finding needs a verifiable disposition produced by the decision tree (详见 `author-response-protocol.md`)：
  ...
  - **强关联发现挂 issue 也是 Let It Slide**（源自历史教训）：发现与本 PR 语义强关联时，「建 issue 留后续」是阻力最小路径的伪装——论证「需要搭 fixture」「非本 PR 文件」不证明承载不能。建 issue 前必须过关联度前置闸，举证「为什么不能现在修」（见 `author-response-protocol.md` 建 issue 前置闸）。
