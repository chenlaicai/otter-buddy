...
**Fix**: Every finding needs a verifiable disposition produced by the decision tree (详见 `author-response-protocol.md`)：
  ...
  - **处置的真实性**：如果你说"建 issue"，你应该真的执行 `gh issue create` 并贴出链接，且按 SYSTEM.md R2 Issue 规范打标（type + priority，标题 `[模块] 摘要`）。没有 issue 链接的"已记录"等于静默丢失——这就是本反模式的变体；无标签的 issue 则让看板筛不出来，是它的轻量变体。
  - **强关联发现挂 issue 也是 Let It Slide**（F20260909ugd8，源自 #860 教训）：发现与本 PR 语义强关联时，「建 issue 留后续」是阻力最小路径的伪装——论证「需要搭 fixture」「非本 PR 文件」不证明承载不能。建 issue 前必须过关联度前置闸，举证「为什么不能现在修」（见 `author-response-protocol.md` 建 issue 前置闸）。
