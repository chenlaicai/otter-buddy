...
6. **自检**：测试通过、符合项目规范、无方案外变更、无兼容桥代码、视觉变更有截图证据、发现的问题全部修复。
   ...
   **pre-existing 声明硬门禁（#614）**：自检报告中的任何「pre-existing / 与本次变更无关」的测试失败声明，必须附验证证据——`git stash` 后基线复跑输出，或 checkout 合入前基线的对照输出。无证据 = 未验证，不得写入自检报告（8/30 #599 现场：5 个自引入失败被误报为与己无关，靠大獭人工核实才兜住）。
