...
# 每日 issue 处理（9:30，F20260917swsh 改造）
...
4. **任意 open 的标签不完整 issue（F20260907itri）**：缺 type 或缺 priority 的，按创建时间从早到晚每日补标 ≤5 条——依据标题+body 语义判断，`gh issue edit <N> --add-label` 补齐；拿不准类型的在 issue 评论留问待人工，不强打
...
1. **已修复但未关闭的 issue（语义级，F20260831whfw）**：扫描**近 7 天合入的 PR**，对其标题/正文与 open issue 做语义匹配——PR 描述用「issue #N」「修复了 #N」等非关键词行文的也要抓到（#566 案例）。命中即留评论说明后关闭。工具：`gh pr list --state merged --search "merged:>YYYY-MM-DD"`（7 天前日期）+ `gh issue list --state open` 逐条语义比对
1b. **认领回收（F20260901cimp）**：扫描全部 open issue 的 otter-claim 认领——认领超 48h 无后续 PR、无评论更新、对应对话无活动的，留问询评论；再过 24h 仍无响应的发 `otter-claim-release` 解除认领。回收记录写进当日产出
2. **daily-review issue 超期关闭**：超过 3 天的 daily-review issue，如果对应问题已在 main 修复，留评论后关闭
3. **stale issue**：超 14 天无更新的 issue（非 daily-review、非 tech-debt——tech-debt 放宽到 30 天），留评论标记 stale 并关闭
...
- 标签不完整率（d/N）>5% 时标红并在次日优先补标；lint 脚本不可用时标注「lint 不可用」，不静默跳过
- 补标不占 actionable 配额，是额外例行职责
