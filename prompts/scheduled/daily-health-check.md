...
4. **self-healing events**：用 `manage_healing_events(action: query)` 查看系统自愈记录 — 工具故障、检索缺失、格式异常都在这里，注意 otterId 字段可定位到具体海獭。**二维分账（#998）**：统计/呈报 errorType 分布时必须按「环境/系统失败」与「獭能力失败」两列分列——口径：tool_failure/rate_limit/circuit_break/self_restart/guard_intercept = 环境，missing_context/wrong_tool/format_violation/knowledge_gap/performance/degenerate/tool_use_feedback = 能力，other 归能力。混排会把工具故障误读成獭不行、把獭不行误读成工具故障（#791 同类：口径混排即数据不实）
