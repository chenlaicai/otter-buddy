...
> ⚠️ 主仓保护（F20260916gsrd）：在主仓目录运行 `stop`/`restart` 且 PID 文件指向运行中的主服务时，脚本会拒绝执行。生产主服务是海獭运行环境，海獭与自动化脚本无合法重启场景——验证代码变更请在 worktree 用独立端口启动隔离实例（见 F20260914dsrv）；确需重启主服务由搭档人工执行。
