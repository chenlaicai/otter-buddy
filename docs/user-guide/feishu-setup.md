...
1. **自动注册**：私聊或群聊里给机器人发第一条消息时，系统自动创建 Connection
2. **手动绑定**（Web 端）：连接管理页 → 创建 Connection，填入飞书会话 ID（`oc_` 开头）

### 助理态与工作态（F20260918imas）

IM 消息分两种模式，由消息来源自动区分：

- **助理态（默认，私聊）**：与机器人的**私聊**（p2p）首条消息自动开一个专属助理对话（标题如「飞书助理 · 张三」），无需任何命令，加好友即聊。对话静默超过 72 小时（可配 `im.assistant.rotationHours`）后，下一条消息会自动翻新篇：旧对话生成收篇摘要存入记忆（海獭仍记得你聊过什么）并收档，新开一篇继续。
- **工作态（群聊 + 命令）**：群聊仍需 `/in` 显式绑定对话（全群共享同一对话上下文）；搭档本人可在私聊里用命令接管任意工作对话。

总开关：`config.yaml` 里 `im.assistant.enabled: false` 可关闭助理态（回退到「未绑定提示 /in」的旧行为）。
