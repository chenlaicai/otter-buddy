...
## 定稿改名与 #886 冲突消解记录（2026-09-14）

- 原 ID F20260912avlb（9-12 创建），按「合入当天日期」定稿规则改名 F20260914avlb（F20260914prdb 三配套）
- 审视闭环回顾：三轮审视（检视-avlb r1 通过 + 建议1 处置 826002dc + delta 复核通过）
- **与 main #886 批次（F20260908rlcp 台账退役 + ctlv entries 切换）的 9 处冲突消解原则**：
  - 旧派发台账（dispatchAttempt/dispatch_attempts/settle/abort 清账）：随 main 退役——#903 的核心是新表 dispatch_records，与退役的旧表是两代机制
  - restart_pending_resumes：随 main 退役（messages 族）
  - 本 PR 核心（dispatch_records 表 + createRecord/markDispatched/markDissolved + query_dispatch_ledger + Web 活动页 + otter_context 一次性迁移）：全部保留
  - yield 工具融合：main 的 createYieldEntry 新路径 + 本 PR 的 updateDispatchLedgerOnYield 记账钩子叠加
  - migration 顺序：main 的 messages→entries 迁移在前，本 PR 的 dispatch 伪存储搬家在后
- 修改的测试文件（speak-tool/yield-level）随 main 删除（其断言对象已退役）
- 解后全量 2895/2895 + tsc 0 错 + web build 通过
