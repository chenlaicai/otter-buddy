...
- **覆盖两张游标表**（检视发现 1）：用户表 + `conversation_participants`
  （獭游标是 `getUnreadEntries` 注入依据，漏修则獭重复消费旧消息）
- `target = MAX(主库当前游标, 映射值)`：MAX 以**主库当前值**为基准（不是
  备份库值）——迁移后用户/獭可能已在新序号空间推进过游标，绝不能写回
- 安全设计沿用 #753 脚本惯例：默认 dry-run / `--apply` 才写 / 执行前
  better-sqlite3 backup API 自动备份 / 单事务 / 前置防呆（备份库须有
  messages 表、主库迁移标记须为 done）/ 异常行告警跳过 / 幂等重跑零更新
...
- 脚本 dry-run：用户游标 162 条（需更新 151）、獭游标 817 条（需更新 806）；
  用户未读预估 2053 → 3，獭未读预估（getUnreadEntries 语义）3049 → 61
- 脚本 `--apply` 演练（主库副本）：更新后幂等重跑零更新（before/after 一致）；
  剩余未读均为真未读（迁移后新消息或用户推进后的新条目）
- 演练产物已清理，生产库未动——**执行 `--apply` 待搭档批准**

## 4.1 检视处置（第 1 轮，检视獭-rmap909 / mimo）

| # | 发现 | 处置 |
|---|------|------|
| 1 | 严重：脚本漏修 `conversation_participants.last_read_seq` | 接受并本 PR 修复（锚点已核实：`sqlite-entry-repository.ts:296-311`）；顺带修正 MAX 基准与幂等性（见 3.2） |
| 2 | 建议：anchorIdOf 线性扫描可二分 | 反驳：单对话消息量 ≤ 数百行、扫描有早退，实测迁移总耗时秒级；一次性路径手写二分引入边界条件风险，收益不可测——改了变差 |
| 3 | 建议：prepared statement 每次重建 | 反驳（与检视者同结论）：better-sqlite3 prepare 有编译缓存、一次性执行开销可忽略；外提需改函数签名，可读性变差 |
| 4 | 建议：备份库默认路径硬编码日期 | 反驳（与检视者同结论）：一次性运维脚本，日期自文档化价值 > 泛化价值；`--backup=` 已提供灵活性 |
