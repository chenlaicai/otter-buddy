...
- 模板示例自检：3 个模板均过自检清单（title 独立支撑决策/结论最前/表格+高亮/被否方案/无前置色块/无写死色值/data-height）；初轮检视后补强：复盘卡 3 秒层已补「置信度」字段（与决策通报/方案对比卡对齐），黑话翻译表补 `otterCard.submit` / `data-height·otterCard.resize` 两条（17→19 条）
