---
id: F20260916hcel
title: html-card 弹性化：合并 html-report + 尺寸弹性
summary: "html-card 单一围栏弹性化——体积 MAX=64KB 无 MIN、新卡默认折叠（cardSchemaVersion=2）、高度 clamp [100,4000] 海獭自控（data-height + otterCard.resize）、删除 html-report 分轨"
change_type: feature
...
supersedes:
  - F20260915hrpt
intent:
  problem: "PR #955 的 html-report 与 html-card 分轨方案被搭档判定「看起来差不多、真实差异只有大小」，且「比例不应定死」——需要合并为单一围栏类型 + 弹性尺寸；搭档后续明确「默认合起来」（折叠）"
  expected_effect: "只有 html-card 一种围栏；体积只限 MAX=64KB（无 MIN）；所有卡默认折叠；高度 clamp [100,4000] 海獭自控；单消息 2 张"
  verify_by:
    type: behavior_check
---

# html-card 弹性化：合并 + 尺寸弹性
...
3. **默认折叠（合起来）**：所有 html-card 默认折叠（搭档 9/16 纠正——之前大獭误执行为「默认展开」）
...
- 所有卡默认 collapsed
- 用户点开展开
- schemaVersion=2 仍写入（供未来可能的默认状态变化用）
...
- `web/src/pages/conversation/HtmlCard.tsx`：高度弹性（clamp [100, 4000]）
...
| `src/interface-adapters/agent-runtime/tools/html-card-contract-tool.ts` | 契约文档更新（data-height、otterCard.resize） | ✅ |
...
| `web/src/pages/conversation/HtmlCard.tsx` | data-height 属性解析+clamp；默认折叠（搭档 9/16 纠正后修正） | ✅ |
