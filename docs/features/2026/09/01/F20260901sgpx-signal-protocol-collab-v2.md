...
业界主流 multi-agent 框架（orchestrator 路由式、RPC 直调式、黑板共享式）共享三个隐含假设：**调度中心做语义判断、打断即丢弃工作、人类是插桩的特殊节点**。本方案将三者全部拆除，换来四个别家不具备的能力：

1. **频谱式投递**：打断还是等待，不再是系统替你二选一。投递方用三档力度（NORMAL/URGENT/HALT）表达急迫度，接收方自决响应；力度是协议、分寸是内容——系统全程零语义判断，只当邮局。
2. **零污染打断协商**：急事到达不截断执行流。被打断的獭在隔离副本中决策「继续/转向」，主上下文零污染，出口一个 bit；被打断的活自动降级为待办（yield-to-self），恢复对称——打断≠丢弃。
3. **注意力卫生内建**：第一个把「上下文注意力」当一等公民保护的协作协议——打断决策 fork 出去干净地做，主 session 永远不被系统噪音灌满。
4. **人獭同权**：用户与獭共用同一套投递协议。「插一句/急事/停下」就是频谱上的不同力度，不是特殊通道——群聊里没人有特权。

一句话：**别的框架把 agent 当线程调度，本方案把 agent 当协作者对待。想说就说、分寸自持、打断有礼——多人群聊的质感，第一次成为协议而非运气。**
...
**「连续」的精确定义**：同一 otter 的 self-yield 链长，中间无 to≠self 信号消费介入——A→A→A→B→A 中 B 消费后链重置，末尾 A 从 1 数起。重置条件：消费了 to≠self 的信号，或被 URGENT 打断决策。计数跨任务累计、真相源 = 消息表（进程重启后重建，不归零，崩溃恢复后护栏仍生效），运行时缓存于 otter 上下文元数据（非对话内容）。
