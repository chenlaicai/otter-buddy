...
- **T2** 频谱式协作：打断/等待/叫停不再是二选一，投递方选力度、接收方自决响应，系统零语义判断
...
│ forkFrom（决策副本）│ navigateTree+summary（合流）  │
└──────────────────────────────────────────────┘
```
...
// to: 獭名 | 'user' | self（自指合法，拆除 talking-stone.ts:41 自交禁令）
```
...
| HALT | N/A | abort+waitForIdle | 无（物理停） | abort（agent-session.js:1165-1169） |
...
async function invoke(otter) {
  ...
  while (next) {
    ...
    if (!next) return;                        // 空箱无下一棒 → IDLE，presence 翻转
  }
}
```
...
② 系统 steer() 注入决策请求（下一工具边界）：
   ...
   不作答 = continue
③ B 若开启注意力卫生 → fork 副本决策（forkFrom 复制完整历史，主 session SUSPENDED 零 token）
   未开启 → 主 session 内联决策
④ 出口结构化：{decision: 'continue'|'pivot', reason}
⑤ continue → URGENT 降级 NORMAL 留箱
   pivot → 系统 auto yield-to-self（旧任务上下文锚点）入箱 → B 围绕新消息重启 invoke
```

- 决策的**过程**树内自由（LLM 任意推理），**出口**必须结构化（flash 裁决，搭档原话「本獭必须给出结论」）
- 注意力卫生开关属接收方（自己的上下文自己管）
- fork 副本保留到对话结束（审计），成本 = 同步文件 IO（毫秒级）+ 短决策调用（~100 token）
...
3. URGENT 不作答兜底：决策请求无响应 → 按 continue 处理 + 留痕
4. HALT 存续：abort 后 yield-to-self 在箱、任务锚点完整
5. 护栏梯度：连续 self-yield 第 3 次收到警示注入、第 5 次强制中断
