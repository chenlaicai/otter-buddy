...
2. **回读退场**（原子性替身）：发认领评论后立即回读全部评论——存在更早的他人 otter-claim 且无 release → 自己输，退场换目标。并发认领靠 GitHub 服务端时间戳（不可伪造）确定性裁决。**硬门**：回读调用失败/结果异常 → 停手报告大獭，不得继续开工；回读发现自身认领评论缺失 → 同样停手核实（审计轨迹被破坏）
...
6. **清理防线**：批量扫尾删 worktree 前必核两证（mtime >48h + 无活跃 otter-claim），任一不成立保留并标注「疑似在途」；认领匹配为 otter-claim 注释 `worktree=<名>` 字段精确匹配，禁止模糊搜短名
