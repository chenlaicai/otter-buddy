...
**R8 收敛（搭档 12:41 追问补齐）**：rebase 整合时 R8（F20260909debr 决策分级与终审简报）原样并入未过三问审查，搭档指出后收敛：根因块（9/9 盲签反馈原话）与标题 F 编号删（根因归 F20260909debr 特性文档）；弹性句删（R3 已覆盖同语义）；终审简报段与 BIG_OTTER.md:62-64 重复（同一规则两处落笔=熵增典型形态），收敛为一句硬规则+模板指针，六要素枚举归 decision-briefing.md 模板自身；L0/L1/L2 分级与默认通过模式三条件保留（行为分级本体）。18 行 → 8 行。
