...
summary: 新增 scripts/ensure-hooks.mjs 检测并自愈失效的 core.hooksPath（.husky/_ 残留、绝对路径覆盖、未配置），prepare 接管为自愈入口，新增 hooks:check/hooks:fix，附真实 git 仓库 + worktree 的 7 用例防回归测试。
change_type: feature
status: implemented
...
- 判定（fail-closed）：`core.hooksPath` 未配置，或按 git 解析规则（相对路径基于仓库根，worktree 即 worktree 根）找不到**可执行的** `commit-msg` → 判失效
...
| `tests/scripts/ensure-hooks.test.ts` | 新增：真实临时 git 仓库 + 真实 worktree 的 7 用例防回归 |
...
| vitest tests/scripts/ensure-hooks.test.ts | ✅ 7/7 | 含 worktree 内检测+修复全局生效用例 |
