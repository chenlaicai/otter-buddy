...
1. troubleshooting/SKILL.md:41 `**修法排序**` → `**修法决策树**（原称「修法排序」，…）`——首次出现处改名+过渡注释，同文件后续引用（:43/:52/:54）与跨文件引用（commit-convention/worktree-isolation/code-implementation 共 6 处）保持「修法排序」字样不动（引用语义不受影响，避免一次 PR 扩散到 5 个文件）。
...
取舍：项 1 只改首次出现处而非全仓替换——全仓 6 处引用同步改会让本 PR 从 3 文件膨胀到 6 文件，且旧称在引用处有「原称」过渡锚点不会造成理解断层；后续触碰各文件时自然收敛。
