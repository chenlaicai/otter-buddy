...
## 追加：requirement-analysis 模板补「未决问题」节（2026-09-17 晚，搭档指令并入本 PR）

**来源**：搭档提供的外部学习总结（spec-kit 系 intent.md proto-spec 方法）对照分析——intent.md 五块（问题/预期结果/受影响方/约束/**未决问题**）中，「未决问题」是 otter RA 模板缺的槽位：允许未知项留空、不要求 AI 猜满。本特性初稿「--config 已存在」假前提被检视证伪正是「空白被编造填满」的实例——若有此槽位，该前提应躺在未决问题里等核验。

**改动**（`.pi/skills/requirement-analysis/SKILL.md`，两处）：
1. 步骤 2 意图确认后新增「未知项留槽，不猜满」指引——未确认前提/未决范围/缺信息项写入「未决问题」节显式留空，「我不知道」是合法状态；禁止用编造前提填满空白
2. 产出模板「非目标」后新增「## 未决问题（可选）」节——无则写「无」

**设计取舍**：可选节 + lint 不强制（机制净新增=不涉及；强制会让所有方案背格式税，留空合法才是这个槽位的价值）。对照分析全文见本特性对话（intent 块/lint/意图锚等其余要素 otter 已有等价覆盖，未决问题槽位是唯一采纳项；「信号→意图升格通道」「漂移率度量」两条留档观察）。
