...
去掉 invocation 级 `checkExecutionTimeout`（从 `ToolCallCircuitBreaker` 中删除 B-4 规则、`startTime` 字段、`maxExecutionTimeMs` 配置），改为在 `attachCircuitBreaker` 中实现 per-event resettable timer：
...
- **resettable**：每次新工具调用重置计时器，只要 agent 在持续工作就不会触发
...
| reason | UI 消息 |
|--------|---------|
| `circuit_break:event_timeout` / `circuit_break:timeout` | 「[系统保护] 单次工具调用超时，已自动中断。」 |
...
- **circuit-breaker-helpers.ts**：`attachCircuitBreaker` 中新增 resettable timer 逻辑；unregister 时清除 timer
...
| B-4 `checkExecutionTimeout` | resettable timer（circuit-breaker-helpers） | 从熔断器类中移出，改为事件层 resettable timer |
...
2. **超时逻辑放在 circuit-breaker-helpers vs ToolCallCircuitBreaker 类**：放在 helpers 的事件层。理由：resettable timer 是事件驱动的时序逻辑，不属于熔断器的「行为模式检测」职责；类只管签名/连续/滑窗，timer 在 subscribe 回调中管理。
3. **UI 消息区分 vs 统一消息**：按 reason 细分。理由：用户（搭档）反馈看不出是超时问题，无法判断是否需要调整配置或报告 bug。

## 测试

- 删除 1 个旧 invocation 超时测试（`checkExecutionTimeout` 已移除）
- 新增 2 个 per-event 超时测试：
  - 单次工具调用超时触发 `circuit_break:event_timeout`
  - 下一次工具调用重置 timer，不误触发
- 新增 1 个 agent-invoker 测试：`circuit_break:event_timeout` 呈现「单次工具调用超时」文案
- `npm run build` 编译通过；`npm test` 全量 694 个测试通过
