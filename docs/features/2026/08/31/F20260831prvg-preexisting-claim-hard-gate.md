...
issue #614 方案段点名「code-implementation / worktree-isolation 的自检清单」，本实现未落 worktree-isolation，理由：①该 skill 无自检段（grep 核实无声明载体）；②其小改动通道（lockfile/配置/文档订正）由 adversarial-review B3 核验侧兜底，无生成侧门禁缺口。

# 处置留痕（#621 检视发现 1）

三落点文本将 `git stash` 收紧为 `git stash -u`（含未跟踪文件），消除「新增测试残留 + 实现被收起 → 基线假失败」的失真证据路径（检视指出该路径恰好复刻 #599 要防的假验证模式）；同时将「checkout 合入前基线」明确为「基于 origin/main 的基线对照」，消除歧义。
