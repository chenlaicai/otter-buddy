...
### 第二轮（2026-08-28，磨石，mimo 异体，审 PR #548 实现）

**焦点**：3 处设计偏离逐条裁决、ViewKey 改名回归、守卫与演练可信度、缺省派生规则一致性。四焦点核验全过，独立跑测 163 文件 / 1959 用例全绿。0 严重 + 3 建议。

| 发现 | 分级 | 处置（决策树） | 理由 |
|------|------|----------------|------|
| 1. TopBar filter 排除参数化页面无 warn（与 icon miss warn 不对称） | 建议 | **确认反驳（零改动）** | filter 排除是清单声明语义（参数化页面默认不进导航，nav 显式声明才进），icon miss 是意外遗漏——两者性质不同，不对称合理；且 map 在每次渲染执行，warn 会随 conversation 详情页正常使用刷屏 |
| 2. MPA_PAGES 顺序与设计稿不一致（health→connections→settings） | 建议 | **确认反驳（零改动）** | T2 目标「现有 7 页面行为不变」优先——按设计稿顺序会导致导航可见顺序变化，违背 T2；设计稿顺序是示意，实现时核对现状后调整 |
| 3. ICONS 用 Record<string, LucideIcon> 而非 Record<MpaEntry, …> | 建议 | **确认反驳（零改动），但修正审视理由 ②** | 结论同意反驳，决定性理由是 ①：取舍表显式声明「icon 是视觉细节，缺配视觉降级可接受」——Record<MpaEntry,…> 要求穷尽，新页面暂无 icon 会直接编译失败，违背取舍。**理由 ② 表述不准确**：ICONS 与 LucideIcon 均在 web 编译域内，改 Record<MpaEntry,…> 不涉 api-contract 包，「污染后端编译域」不成立；不影响处置结论 |

> 处置小结：3/3 确认反驳，零改动。发现 3 的反驳结论正确但理由 ② 依据不实，实现者按事实修正理由、保留处置。
...
### 三决策点拍板（搭档 chen，2026-08-27）

1. 清单落点：`api-contract/web/pages.ts`（推荐项生效）
2. label 进清单（推荐项生效）
3. #479 随方案关闭（推荐项生效）

### 与设计的差异（3 处，均为设计稿示例的疏漏修正）

1. **TopBar 排除规则**：设计稿消费方 3 示例直接 `MPA_PAGES.map(...)` 未排除 conversation 详情页（它无 nav 且 pattern 带参数），按示例实现会把「对话详情」渲染成第 7 个导航项（现状从不存在，T2 行为不变目标违背）。实现加 `.filter(p => p.nav !== undefined || !p.pattern.includes(':'))`：带参数且未声明 nav 的页面不进入导航。
2. **清单顺序迁就现状**：设计稿清单顺序（…settings、connections、health）与 TopBar 现状可见顺序（…health、connections、settings）不一致，若按设计稿顺序导航顺序会变。实现把清单顺序调为与现状一致（health 在 connections 前）——本 PR 不改变任何页面行为。
3. **icon fallback 的 console.warn 写法**：设计稿写为 `ICONS[p.entry] ?? (console.warn(...), MessageCircle)` 逗号写法，实现改为 map 内先取 `configured` 再 if-warn 再 return（可读性优先，语义等价）。

### 实现清单

| 文件 | 操作 | 说明 |
|------|------|------|
| api-contract/web/pages.ts | 新增 | MPA_PAGES 清单 + MpaPage 类型 + MpaEntry 派生类型（含 JSDoc 消费方清单） |
| web/vite.config.ts | 修改 | 函数式配置，input 从清单生成（相对路径 import，因 alias 定义在本文件内） |
| src/bootstrap/server.ts | 修改 | 7 行手写注册 → 3 行循环（@contract alias import） |
| web/src/components/TopBar.tsx | 修改 | ViewKey 从清单派生 + tabs 映射（含排除规则）+ icon 表局部保留（fallback + console.warn） |
| web/src/components/AppLayout.tsx | 修改 | 手写 union 副本 → re-export TopBar 的 ViewKey（第 6 处副本消除） |
| conversation-list + conversation 页面 | 修改 | 5 处 activeView 字面量 "conversation"→"index"（保高亮行为） |
| tests/bootstrap/server-static-routes.test.ts | 修改 | it.each 生成化 + 夹具页面集合生成化 + html 集合守卫（第 5 处漂移点） |

### 验证结果

1. **回归** ✓：路由测试 11 用例全绿（原 10 用例生成化后语义不变 + 尾斜杀 404 / 未注册 404 / staticRoot=false 三个行为断言保留）
2. **T1 演练** ✓：清单加假页 fake + web/fake.html（不改任何其他文件）→ ① 路由用例自动增至 12 个且绿（/fake 200）② vite build 产物含 fake.html；TopBar 为纯前端派生（清单含 fake 即出现「假页」项，构建机制同构）
3. **T3 反向** ✓：撤假页后路由用例回 11 个、vite 产物回 7 入口，全部同步消失
4. **守卫自证** ✓：只加 web/orphan.html 不登记清单 → html 集合守卫红；删除 → 绿
5. **双域类型检查** ✓：根 tsconfig + web tsconfig `tsc --noEmit` 均 0 error；`vite build` 成功（7 入口）
6. **全量** ✓：主仓 163 文件 / 1959 用例全绿（中途一次 halt-injection-wiring 超时为偶发，单独跑与重跑均绿）；web 28 文件 / 242 用例全绿

### 实现期环境备忘（不影响产物）

- worktree 初建时 web/node_modules 与根 node_modules 未安装（worktree 不共享主仓的），已 npm install 补齐；根侧 busboy 缺失源于新 main 带入的 #538 依赖，同样 npm install 解决
