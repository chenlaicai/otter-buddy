...
R20260828pntr（ponytail 深度研究，PR #552 已合入）提炼了四条可迁移思想，其中两条适合
立刻以轻量 prompt 改动落地（P0 为 golden selftest 层，见 F20260828gssf / PR #556）：

1. **过度建设偏差**（§0）：LLM 训练偏差天然导致"我要一个函数，它给我一个框架"
   （date picker 404 行 vs `<input type="date">` 23 行）。ponytail 的核心是用 7 级阶梯
   （YAGNI → 仓库已有 → stdlib → 平台原生 → 已装依赖 → 一行 → 最小实现）在第一个
   能站住的台阶停下。
2. **批评→测试用例**（§2.5 / §5.3）：Colin Eberhardt 的四点批评被 ponytail 作者转化为
...
| 文件 | 改动 |
|------|------|
| `.pi/skills/code-implementation/SKILL.md` | 步骤 6 自检清单新增「最简实现检查」必答项：先过阶梯（仓库已有实现 → stdlib/平台原生 → 已装依赖 → 才写新代码），更简且不改语义则采简弃繁，确认已最简则记录结论 |
...
- **为何 golden 场景要"含 good/bad 参考轨迹"**：F20260828gssf 已把 selftest-first
  落进 runner——新场景自动获得零 LLM 判别力校验，两条改动互相咬合。
...
- 锚点核对：R20260828pntr §0（阶梯/过度建设）、§2.5（#126 案例）、§5.3（可迁移思想
  列表）均直接读原文核对，非凭记忆
- `npm test` 不受影响（纯文档改动，无代码路径）——CI 将复验
