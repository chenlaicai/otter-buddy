...
2. **重名兜底**：创建前查 `conversation_otters`/participants 在场同名 active 小獭，命中抛 DomainError（409）。错误信息附在场同名者 ID，与大獭工具链措辞对齐。
...
头像           九宫格（3×3，像素 SVG 直接渲染）+ 首项「随机」（默认选中，_dice icon）
               选中即预览放大 + 意象名标注（獭祭鱼/竹笠/朱结/眠月/抱贝/衔竹/墨痕/莲叶/葫芦）
...
| 血缘解析放哪层 | controller（HTTP 入口特有） | usecase 通用解析 | 大獭工具入口血缘天然可信（系统注入），解析逻辑只有 UI 入口需要；放 controller 避免污染通用 usecase 契约。usecase 只加「接受 conversationId 可选参数」的薄口子 |
...
- **契约单测**（vitest，src 侧）：controller create() 透传 modelAlias/avatar；非法 modelAlias 400（含可用列表）；非法 avatar 400；conversationId 血缘解析（单大獭/双大獭取最早/无大獭）；前端传 parentOtterId 被忽略；usecase 重名兜底 409。
- **repo 单测**：createOtter 写 avatar、getById 读回、旧行 avatar NULL。
- **前端单测**（vitest，web 侧）：CreateOtterModal 渲染（九宫格/模型下拉/随机默认项）；提交 body 组装（含 conversationId、不含 parentOtterId）；重名预检 toast；prompt 引导生成模板结构断言。
...
| `src/usecases/otter/create-otter.ts` | 修改 | input + avatar/conversationId；重名兜底；血缘消费 |
| `src/interface-adapters/http/controllers/otter-controller.ts` | 修改 | 透传 + modelAlias/avatar 校验 + 血缘解析 |
| `src/entities/otter/otter.ts` | 修改 | Otter + avatar?: string \| null |
| `src/frameworks/db/schema.ts` | 修改 | otters 表幂等 ALTER 加 avatar 列 |
...
| `web/src/pages/conversation/Modals.tsx` | 重做 | CreateOtterModal（T3 字段结构） |
