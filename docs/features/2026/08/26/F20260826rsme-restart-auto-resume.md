...
1. `sender_type = 'otter'`（用户消息不恢复）
2. 所属 conversation 仍存在
3. 该 otter 仍是 conversation 的 active participant（已解散对话不恢复）
4. 原子 attempts 守卫通过（防循环，见上）

`failInFlightMessages` 签名扩展：接收可恢复消息 id 集合，集合内消息只置 `failed` 不插 notice——恢复流程会重置回 streaming 续写，notice 无意义且污染续写内容。
...
resume():
  ...
  4. 逐条串行处理：
     ...
     b. senderId = 同 turn 最后一条 user 消息的 senderId，fallback "user"（对齐 retry 端点）
     c. prepareForRetry(messageId, preserveSegments=true)
        → 消息 failed→streaming，新 turn，半截 segments 保留（F20260821fix 同款语义）
     d. dispatchChainEngine.executeChain({
     ...
     e. 成功 → status=done；链引擎抛错 → status=exhausted + 补插失败 notice
...
2. **恢复 invoke 与用户新消息并发**：同 conversation 两个 invoke 存在 sequence_num 竞态可能。缓解：3s 延迟基本错开；彻底解法是 conversation 级互斥锁，超本特性范围，标注为已知限制。
...
| AT-3 | 防循环 | 恢复 invoke 进行中再次重启 | 第二轮 attempts 守卫拒绝入队，走现状 fail+notice，不二次恢复 |
...
| AT-6 | 开关关闭 | buildApp({ startResume: false }) | reconcile 正常入队但 resume 不执行（测试场景）——生产路径 reconcile 侧也有开关联动（startResume=false 时 reconcile 直接走现状） |
