...
summary: |
  W0 清理 Part A（PR-1）：skills 页撤假 CRUD，数据源换 manifest 真实 skill，加整页"建设中"横幅；Part B（PR-2，pending）：lint-skills/lint-tool-manifest 接线进 CI。
...
本档为 #366 优化第一轮 Wave 0 清理类共用薄档：

- **Part A（PR-1，已完成）**：skills 页撤假 CRUD + 建设中标注
- **Part B（PR-2，pending）**：lint-skills / lint-tool-manifest 接线进 pre-commit + CI，待追加
...
Part A（PR-1，纯前端展示层），两轮定稿——初版撤假 CRUD + 详情区横幅，经对抗审视后修正：
...
Part B（PR-2）：lint-skills / lint-tool-manifest 接线进 pre-commit + CI。
...
| D2 一档跨两 PR 验收表混装 | 中 | 采纳：Part A/B 结构化 | 已修 |
...
- **Part A/B 薄档结构**：Part B 合并时追加内容并按 Part 分列验收，档状态在 Part B 落地后推进。
