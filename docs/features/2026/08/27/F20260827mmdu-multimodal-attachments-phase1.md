...
summary: |
  ...
  ≤2 图硬限制，retry 带原附件）；FTS 写入时序修复；models[] 可选 input。41 文件 +2861/-216
  （含三轮 merge main 后的净 diff），163 测试文件 1958 用例全绿。
...
全量：163 测试文件 1958 用例全绿（我方新增 55+12+1=68 用例 + 三轮 merge main 带入约 143）；eslint 0 error；tsc 干净。
...
| 严重6 | 文档数字失实（25 文件 vs 37） | ✅ 本文档按实际重写（git 实测 41 文件 +2861/-216，含三轮 commit+merge；后续以 `git diff origin/main...HEAD --stat` 为准） |
...
41 文件 +2861/-216（三轮 commit + 三轮 merge main 后的净 diff，git 实测）。新增源文件：attachment.ts / attachment-projection.ts /
