...
### 第二轮：实现代码审视（2026-08-27，磨石，mimo 异体）

**焦点**：守卫测试免疫性、删除完整性（6+1 目标 vs diff）、FTS5 影子表过滤正确性、三处设计偏离裁决。
**结论**：0 严重 + 3 建议（全注释补丁类）；四项焦点核验全过；磨石独立跑测 155 文件 / 1857 用例全绿。

| 发现 | 分级 | 处置（决策树） | 理由 |
|------|------|----------------|------|
| 1. 守卫测试 listTableNames 与 schema.ts 同名函数语义差异无说明 | 建议 | 接受并修复：补 JSDoc 说明差异原因 | 未来维护者会疑惑为何两份同名函数行为不同，注释成本极低 |
| 2. vtSql 拼接绕开 lint-tests 无代码内注释 | 建议 | 接受并修复（与发现 1 同区域合并修）：行内注释扩充为完整解释（误报正则 + 注释行被剥离的事实） | 原有括号内注释只提误判未解释机制；另核实 lint-tests.mjs:42 剥离注释行，注释内可直写敏感字样 |
| 3. eslint-disable 恢复指引注释位置 | 建议 | **反驳保持现状**（磨石自判同向） | 注释位置与文件头部 eslint-disable 原位置同区域，ESLint 报错时自然看到；零改动 |

> 处置小结：2 修 + 1 确认反驳。发现 1/2 的 JSDoc 同时补充了 lint-tests 事实依据（lint-tests.mjs:42 匹配前剥离注释行），使注释本身可被未来读者独立验证。
