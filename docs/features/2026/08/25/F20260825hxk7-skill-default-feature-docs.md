...
- cross-skill 一致性：两个 skill 的产出表对「特性文档随 PR 接受 B2 检核」这一动作的执行者、措辞逐字对齐（审视轮次 1 修复，检视发现 1）
- 本 PR 自身以身作则：交付本特性文档，与 skill 改动同 worktree 提交

## 对抗审视记录

### 第 1 轮（2026-08-25，检视獭-451，mimo）

0 严重 / 3 建议，逐条处置：

| 发现 | 判断 | 处置 |
|---|---|---|
| 1. 两 skill 产出表特性文档行执行者不一致（当前獭 vs 检视獭） | 改了更好——产出表「执行者」列语义是后续动作执行方，审查动作应为检视獭 | **接受并修复**：执行者改检视獭，措辞与 worktree-isolation 逐字对齐 |
| 2. 特性文档 frontmatter 缺模板定义的 doc_type/causal_links 等字段 | 改了变更差——8/24-8/25 共 20 篇统计：doc_type 仅 3 篇、causal_links 仅 2 篇在用，本格式 17/20 为绝对主流；capability_test 是 lint:capability 现行约定（63 篇旧文档因缺它被警告）非「多出」 | **反驳**：维持主流格式；「模板清单与主流脱节」部分接住建 issue #455 |
| 3. 验证节仅声明 intra-skill 一致性，缺 cross-skill 一致性声明——发现 1 恰证明此缺口 | 改了更好——验证声明应覆盖实际修改范围 | **接受并修复**：验证节追加 cross-skill 一致性校验项 |
