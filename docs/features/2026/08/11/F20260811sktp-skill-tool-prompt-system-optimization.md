...
**Magic Words 5 关键词测试明细（mimo-v2.5-pro 实测）**：

| 关键词 | mimo 通过率 | 状态 |
|---|---|---|
| 停下 | 3/3 ✅ | 干净 speak "停下，等你指示" |
| 绕路了 | 3/3 ✅ | LLM 重新审视方案 |
| 就这样 | 3/3 ✅ | A4 流程终止 |
| 星星罐子 | 2/3 ✅ | 立即停止副作用 |
| 严肃点 | **0/3 ❌** | **mimo 已知不稳定**——LLM 在 companion 闲聊上下文中惯性太强，不识别切换信号，继续闲聊 |

「严肃点」语义最抽象（"切换模式"），mimo 在闲聊上下文识别弱。其他 4 个关键词都有具体动作（停止/审视/终止），mimo 能可靠执行。**这是模型层限制不是设计层 bug**——测试 `it.skip` 标记，待模型升级或换更强语义的关键词后启用。SYSTEM.md 中保留「严肃点」定义，等更可靠模型验证。
