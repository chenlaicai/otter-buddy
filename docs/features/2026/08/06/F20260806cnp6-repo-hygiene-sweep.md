...
summary: |
  代码仓定期排查清理：死代码/兼容 shim 物理删除 + 单模型兼容路径移除（Incompatible）+ settings 页对接真实模型体系，一次排查一个文档。
  动机：仓内积累了一批"保守保留"的过渡产物——双定义、双写、结构兼容拷贝、不可达分支，腐蚀唯一真相源原则；settings 页 LLM 区块与真实配置体系完全脱节（假 UI、写入无消费者）。
  机制：全部物理删除或合并到规范位置，不留 @deprecated 桥；llm.models[] 收编为配置唯一真相源；ModelPool 支持运行时默认切换，settings 页改为展示 models[] + 切换默认模型。

causal_links:
  from:
    - F20260717wx6q   # pi-agent-core-cleanup（system-prompt-config.ts 当年"保守保留"决策，本次推翻）
    - F20260731-multi-model-routing   # 多模型路由引入时的单/双路径兼容设计 + settings 语义劣化起点，Part 2/3 收编
...
全仓代码新鲜度检视（兼容性处理、老旧残留、TODO/FIXME、双实现）后的统一清理。分三个 Part：Part 1 死代码与兼容 shim，Part 2 单模型兼容路径移除（破坏性变更），Part 3 settings 页对接真实模型体系。检视中确认**无需处理**的项列在文末。
...
`_ensureModelRuntime` 中的单模型 else 分支（"兼容旧逻辑"）在生产装配下不可达：`initModels` 恒产出 `ModelPool`，bootstrap 必装配下传。删除 else，注释说明不变量。
...
| 位置 | 改动 |
|------|------|
| `config-service.ts` | `AppConfig.llm` 收窄为 `{ default: string; models: ModelConfig[] }`；`RawConfig.llm` 删顶层字段；`validate` 删单模型分支、强制 models[] 非空；`validateMultiModel` 更名 `validateModels` 并删除回填双写；loadConfig 日志改记 defaultModel/modelCount |
| `models-factory.ts` | 删 `initSingleModel` 与 `isMultiModel` 分流，`initModelPool` 成为唯一路径；`loadProvider`/`needsCustomProvider` 参数收窄为 `{ apiKey?, apiBaseUrl? }` |
...
## Part 3：Settings 页对接真实模型体系

### 背景

settings 页 LLM 区块与真实配置体系完全脱节（评审发现的历史遗留）：
- 后端 `SettingsConfig.provider` 在多模型时代即返回 default alias 而非真实 provider 名（F20260731 引入的语义劣化）
- 前端 provider `<select>` 写死 openai/anthropic/google，alias 查不到时模型下拉空白
- testConnection 是 `setTimeout` 假实现；apiKey 写入 settingsRepo 后无任何消费者
- 本质：假可写 UI，write-only 死循环

### 改动

**ModelPool 支持运行时默认切换**（`model-pool.ts`）：
- 新增 `setDefaultAlias(alias): void`（alias 不存在时抛错），default 字段改为 mutable
- 切换只影响之后新建的 session，既有 session 保持原模型

**settings API 对接 ModelPool**（`settings-controller.ts`）：
- `GET /api/settings`：返回 `models[]`（alias/provider/model/description/strengths/weaknesses/contextWindow，**apiKey/apiBaseUrl 不下发**）+ `defaultModelAlias`
- `PUT /api/settings`：接受 `defaultModelAlias`，校验合法性后持久化到 settingsRepo 并即时切换运行时默认
- 删除 `SettingsConfig.provider`/`model` 字段（已无语义）

**分层端口**（`usecases/ports/model-pool-like.ts`）：
- 定义 `ModelPoolLike` 接口供 settings-controller 使用，避免 interface-adapters → frameworks 层级违规

**启动时覆盖应用**（`bootstrap/database.ts` + `main.ts`）：
- `applyDefaultModelOverride`：读 settingsRepo 的 `llm.defaultModelAlias` key，若存在且 alias 合法则应用覆盖；不存在或已失效时告警并忽略
- main.ts 在 `validateModelAliases` 之后调用

**前端 LLM 区块重写**（`web/src/pages/settings/index.tsx`）：
- 删除：modelsByProvider 硬编码、provider/model/apiKey 三个 select/input、testConnection 假实现
- 新增：默认模型 `<select>`（展示 alias + provider/model）+ 模型清单只读展示（description、strengths/weaknesses tags、contextWindow）
- 保存逻辑：`PUT /api/settings` 只发 `defaultModelAlias`

**契约更新**（`api-contract/api/settings.ts`）：
- `SettingsDTO`：删 `provider`/`model`，加 `models: ModelInfoDTO[]` + `defaultModelAlias`
- `UpdateSettingsRequestDTO`：删 `provider`/`model`，加 `defaultModelAlias?: string`
...
- `tests/api/helpers.ts` 两处 TODO mock stub。
...
- settings 遗留已纳入本 PR Part 3 修复（评审发现 → 采纳 → 实现）。

## 验证

- `npx tsc --noEmit`（后端 + web）通过
- `npm run lint` 通过
- `npx vitest run`：86 文件 / 1054 测试全绿（含 settings API 新测试 3 条 + model-pool setDefaultAlias 测试 2 条）
