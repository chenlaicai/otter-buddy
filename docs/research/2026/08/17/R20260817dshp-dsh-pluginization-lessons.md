...
> **快照时点说明**：3.1/3.2 现状摸底取自 2026-08-17 上午（批次3 动工前）。当日批次3 已推进：#283 完成 broadcaster 解耦（"UI/IM 通道"行的 #281 已闭合）；R20260817arnt 设计锁定 + PR-A（#285）合入，ports/ 收拢与服务型 port 声明已落地——3.2 表中"agent loop 无接缝 / sessions 无接缝"两行将由 PR-B/C/D1/D2 依序消除。本研究的增量价值相应后移至：A3 离线比对工具（服务剩余 PR 的验证面）、A4/A5（批次3 后的组合时/per-agent 插件化）、以及三形态图景与已知局限作为长期参照。
