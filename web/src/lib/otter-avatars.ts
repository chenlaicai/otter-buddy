 ...
 *
 * 分配规则：
 * - 大獭（type='big' 或已知大獭 ID）→ datu.svg 固定
 * - 用户 → user.svg 固定
...
/** 已知大獭 ID 池（与 otter-colors.ts 的 BIG_OTTER_COLOR 匹配逻辑一致） */
...
/** otterId → 头像 URL（大獭固定，小獭按 ID 确定性 hash 落入九款池） */
export function getOtterAvatar(otterId: string): string {
  if (BIG_OTTER_IDS.has(otterId)) return BIG_OTTER_AVATAR
