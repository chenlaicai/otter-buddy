...
function main() {
  ...
  if (!Number.isFinite(rawDays) || rawDays <= 0) {
    console.error(`--days 必须为正数，收到: ${args.days}`);
    process.exit(1);
  }
  const since = new Date(Date.now() - rawDays * 86400_000).toISOString();
  ...
  const withCtx = rows.filter(r => r.ctx_window_used != null).length;
    ...
    // 分组聚合（任务类型=对话标题关键词粗分，定时/手动不可拆分见上）
  const groups = new Map(); // key = `${model}|${task}`
  for (const r of rows) {
    const model = r.model ?? "unknown";
    // 任务类型=对话标题关键词粗分。注意：invoke 级触发源（定时/手动）不可判——
    // trigger_entry_id 实测恒 NULL（schema 注释已预告），entries 特征匹配只能到对话级，
    // 同对话内手动/定时混杂无法拆分（本 PR 不新增此维度，根治需写入侧补 trigger 记录）。
    const KIND_RULES = [
      [/雷达|radar/i, "雷达简报"],
      [/体检|health/i, "每日体检"],
      [/外部洞察|洞察/i, "洞察讨论"],
  ...
  const report = [...groups.values()].map(g => {
    ...
    return {
      model: g.model,
      task: g.task,
      n: g.secs.length > 0 ? g.secs.length : Math.max(g.ctx.length, g.input.length),
  ...
  console.log(`成本口径：不含 cache 读写（cacheRead/cacheWrite 未落库）——大量使用 prompt cache 的本系统下为方向性低估，相对排序可能失真`);
  console.log(`分组口径：任务类型=对话标题关键词粗分；invoke 级定时/手动触发源不可判（trigger_entry_id 未落库），同对话混杂轮次归同组\n`);
  ...
  // 汇总：总成本估算（分母对齐：乘 input 有效行数而非组样本数）
  let totalCost = 0, pricedN = 0;
  for (const g of groups.values()) {
    const avgIn = avg(g.input), avgOut = avg(g.output);
    const price = PRICING[g.model];
    if (price && avgIn != null && avgOut != null && g.input.length > 0) {
      totalCost += ((avgIn * price[0] + avgOut * price[1]) / 1e6) * g.input.length;
      pricedN += g.input.length;
    }
  }
