...
export interface AgentRunResult {
  ...
  /** F20260918uhuc：synthesizePast 透传（restart_otter 工具参数——獭决定前世是否值得合成） */
...
/** F20260918uhuc：jsonl entries 读取门面（统一交接原料源——可选，mock 缺省时调用方降级） */
...
export interface SdkInvokePort {
  ...
  /** F20260912nlb896（#896）→F20260918uhuc 泛化：统一合成影子通道——临时 inMemory session 直调 LLM。
  ...
  /** F20260918uhuc：统一交接——获取 per-otter 锁（冻结窗口；waiter 超时交接模式 120s）。可选 */
  acquireSessionLock?(otterId: string): Promise<() => void>;
  /** F20260918uhuc：统一交接——只读当前 session 全部 entries（jsonl 切片原料；空/缺失返回 undefined）。可选 */
