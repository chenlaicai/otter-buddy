...
/** 创建 healing 对话并返回结果 */
async function createHealingConversation(
  deps: {
...
export async function ensureHealingConversation(deps: {
  ...
  // 3. 创建新对话（失败时清理 pending 值，避免阻塞其他进程）
  try {
    return await createHealingConversation(deps);
  } catch (err) {
    ...
    if (currentValue?.startsWith('pending:')) {
      await deps.settings.tryDeleteIfValueMatches(HEALING_CONVERSATION_KEY, currentValue);
      deps.logger.info('Cleaned pending lock after creation failure', { key: HEALING_CONVERSATION_KEY });
    }
    throw err;
  }
}
