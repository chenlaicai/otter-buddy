...
export class AgentTurnOrchestrator {
  ...
  /** F20260922txes：超时类重试耗尽终态判定——可重试 guard 原因（超时/工具异常）+ retryCount>0，
   *  即 L1 自动重试已发生仍失败、走到 L2 硬中断。bash_safety 走 #731 bounce 升级，不在此列。 */
  private isTimeoutRetryExhaustedTerminal(ctx: TerminalContext): boolean {
    return ctx.kind === 'guard'
      && !!ctx.guardReason
      && !ctx.guardReason.startsWith('bash_safety:')
      && isRetryableGuardAbort(ctx.guardReason)
  ...
  private async escalateTimeoutRetryExhausted(ctx: TerminalContext): Promise<void> {
    ...
    ctx.callbacks.recordHealingEvent({
      ...
      severity: "medium",
      description: `超时类自动重试耗尽终态（guardReason=${guardReason}, retry=${ctx.input.retryCount}）：L1 自动重试后仍失败，已硬中断`,
      suggestion: "临时服务波动可手动重试该消息；若持续出现请排查模型服务状态",
