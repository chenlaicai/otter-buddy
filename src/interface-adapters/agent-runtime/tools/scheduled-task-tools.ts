  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        description: { type: "string", description: "任务的人类可读描述（≤500 字符），面板优先显示。一句话说明这个任务干什么，给搭档看的。推荐填写——不填则界面只能看到 body 开头，function 型任务会一片空白。" },
    ...
    execute: async (_id: string, params: Record<string, unknown>): Promise<ToolResponse> => {
      ...
      try {
        const task = await manageScheduledTask.create({
          ...
          description: params.description != null ? (params.description as string).trim() : undefined,
