...
⚠️ **硬性限制**：一条消息最多 ${CARD_MAX_PER_MESSAGE} 张卡片，单卡 ≤${CARD_MAX_BYTES / 1024}KB。第 3 张起用户会看到降级的源码块（不可读）。如果需要展示超过 2 张卡片的内容，请合并为 2 张，或分多次 speak 输出。

## 语法骨架

卡片以 CommonMark 围栏块嵌入 speak 的 body，一条消息最多 ${CARD_MAX_PER_MESSAGE} 张，单卡 ≤${CARD_MAX_BYTES / 1024}KB（超限会被截断，可能导致发言损坏重来）：

\`\`\`html-card title="卡片标题"
<!-- 自包含 HTML 片段：内联 <style> + 结构 + 内联 <script>（可选） -->
\`\`\`

- title 属性必填，用双引号；用户看到的是标题栏
- 卡片 HTML 内含三反引号时，外层围栏用四个反引号
- **所有卡默认折叠**——用户点开展开，消息流干净
- 高度可选：卡片根元素上加 \`data-height="800"\` 属性声明初始高度，系统 clamp [100, 4000]；不传则默认 240px
- 卡片脚本可动态调 \`otterCard.resize(800)\` 声明高度（系统 clamp [100, 4000]），适合内容加载后自适应
- 【强制】围栏必须完整写进 speak 的 body 参数：写在 speak 之外文本里的卡片不会进入消息、搭档看不到，系统会检测并拒绝该次 speak
...
卡片脚本可动态声明高度（系统 clamp [100, 4000]）：

\`\`\`js
// 内容加载完成后自适应高度
otterCard.resize(document.body.scrollHeight)
\`\`\`

也可以用 \`data-height\` 属性在 HTML 根元素上静态声明初始高度：

\`\`\`html
<div class="report" data-height="800">
  <!-- 卡片内容 -->
</div>
\`\`\`
