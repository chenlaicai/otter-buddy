...
import { createGetHtmlReportContractTool } from "./html-report-contract-tool";
...
function createSpeakTool(ctx: ToolContext, healingRepo?: HealingEventRepository, logger?: Logger): AgentTool {
  return {
    ...
    execute: async (_id: string, params: Record<string, unknown>) => {
      ...
      const bodyError = validateSpeakBody(ctx.getTurnAssistantText?.(), cleanBody, ctx.currentModelMaxTokens);
...
function createSetContextTool(ctx: ToolContext): AgentTool {
  return {
    name: "set_context",
    description: "设置当前 Otter 的上下文键值对. When: 需要持久化任务进度/临时变量，供后续轮次读取. Not for: 长期记忆 → search_memory/create_linked_resource. Output: 设置成功确认. BOUNDARY: otterId 由系统注入. TIP: 适合存下轮要用的中间状态；多轮任务随手维护 task_status / next_step 两个 key（任务状态变化时更新前者，每完成一个子步骤更新后者）.",
    parameters: {
      type: "object",
      properties: {
...
export function createTools(ctx: ToolContext, healingRepo?: HealingEventRepository, logger?: Logger, workspaceGateway?: WorkspaceGateway, manageScheduledTask?: ManageScheduledTask, paperLedger?: { ledger: Ledger; getAccountId: () => string | undefined }): AgentTool[] {
  ...
  const tools: AgentTool[] = [
    ...
    createGetHtmlReportContractTool(),
