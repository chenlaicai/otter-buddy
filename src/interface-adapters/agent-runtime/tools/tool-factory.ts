...
function createQueryDispatchLedgerTool(ctx: ToolContext): AgentTool {
  return {
    ...
    description: "查询派工台账. When: 大獭汇报任务状态前核对实际派工记录，消灭状态虚报. Output: 派工记录列表（otterName/task/状态/时间戳；状态为客观生命周期：created=就位待命、dispatched=已派工、dissolved=獭已解散，不含「任务完成」判断——完成真相看对话汇报）. BOUNDARY: 只读不修改状态. conversationId 由系统注入.",
    parameters: {
      type: "object",
      properties: {
        status: {
        ...
        conversationId: {
          type: "string",
          description: "按对话 ID 过滤（可选，缺省查当前对话）",
        },
      },
    },
    execute: async (_id: string, params: Record<string, unknown>) => {
      const records = await ctx.client.dispatch.queryRecords({
        conversationId: (params.conversationId as string | undefined) ?? ctx.conversationId,
