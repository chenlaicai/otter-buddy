...
describe("migrateDatabase - F20260915desc scheduled_tasks.description 列", () => {
  ...
  it("F20260920stkx：paper-trading 回填已移除——老库存量任务描述保留原值不被清空", () => {
    const db = new Database(":memory:");
    try {
      initSchema(db);
      ...
      db.prepare(
        "INSERT INTO conversations (id, title, status, created_at, updated_at) VALUES ('conv-1', '旧实验', 'active', '2026-09-15T00:00:00Z', '2026-09-15T00:00:00Z')",
      ).run();

      const insert = db.prepare(`
        INSERT INTO scheduled_tasks (
          id, conversation_id, name, cron, timezone, body,
          talking_stone_passed_to, sender_id, status, created_at, updated_at, description
        ) VALUES (?, 'conv-1', ?, '5 15 * * 1-5', 'Asia/Shanghai', '{}', '[]', 'system', 'active', '2026-09-15T00:00:00Z', '2026-09-15T00:00:00Z', '历史描述')
      `);
      ...
      // 回填清单置空后：迁移不清空已有描述（历史数据不动）
      expect(t1.description).toBe("历史描述");
    } finally {
      db.close();
    }
  });
