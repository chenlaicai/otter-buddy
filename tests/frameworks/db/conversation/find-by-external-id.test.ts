...
describe('SqliteConversationRepository.findByExternalId', () => {
  ...
  beforeEach(() => {
    ...
    // 含 externalId 的消息
    db.prepare(`
      INSERT INTO messages (id, conversation_id, sender_type, sender_id, status, body, sequence_num, turn_id, talking_stone_passed_to, source, metadata, created_at)
      VALUES (?, ?, 'system', 'boss-bridge', 'completed', ?, 1, ?, NULL, NULL, ?, ?)
    `).run(
      'msg-1', 'conv-1', '[招聘消息批次]', 'turn-1',
    ...
    `).run('msg-2', 'conv-1', 'hello', 'turn-1', '2026-01-02T00:00:00Z');
  });
