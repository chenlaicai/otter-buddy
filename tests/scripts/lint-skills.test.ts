/**
 * F20260903srfs / issue #726：lint-skills.mjs 校验 9（references 引用形态可见性）测试。
 *
 * 校验 9 两条规则：
...
function skillBody(opts: { wfRef?: string; indexRef?: string; absRef?: string }): string {
...
describe("lint-skills 校验 9：references 引用可见性（#726）", () => {
  ...
  it("E2: 同一文件不同引用字符串（references/x vs review/references/x）按解析后路径归一", () => {
    const r = runLint({
      review: {
