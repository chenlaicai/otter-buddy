...
describe("SearchEngine - RRF 融合与单源构建", () => {
  ...
  describe("rrfFusion()", () => {
    it("仅 FTS 命中时 source='fts'，分数 = 1/(k+rank+1)", () => {
      ...
      expect(hit1.rrfScore).toBeCloseTo(1 / 61);

      // rank 1: 1/(60+1+1) = 1/62
      const hit2 = result.get("e2")!;
      expect(hit2.source).toBe("fts");
      expect(hit2.rrfScore).toBeCloseTo(1 / 62);
    });

    it("仅 Vec 命中时 source='vec'，分数 = 1/(k+rank+1)", () => {
      const e1 = makeEntry({ id: "e1" });
      const e2 = makeEntry({ id: "e2" });
      const vecHits = [
    ...
    it("重叠命中时 source='both'，分数累加", () => {
      ...
      // 两个 1/61 累加 = 2/61
      expect(hit.rrfScore).toBeCloseTo(2 / 61);
    });
