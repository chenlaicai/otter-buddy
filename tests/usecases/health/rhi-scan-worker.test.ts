...
describe("RhiScanWorker（临时仓库 + 真 sqlite）", () => {
  ...
  it("snapshotSink 注入后 scanOnce 写入指标快照（F20260829hviz Fix A）", async () => {
    ...
    // 19 行：11 标准行 + 1 chain_states 行 + 6 health_index 行（5 维度 + overall，issue #595 PR1）
    // + 1 trend 行（bugfix_interval 修复半衰期，Issue #645）
    ...
    // Issue #645 修复半衰期 trend 行：3 个 bugfix 在 5 天窗口内，间隔趋势可判
    const trend = rows.find(r => r.metricType === "trend" && r.metricKey === "bugfix_interval");
    expect(trend).toBeDefined();
    const trendMeta = JSON.parse(trend!.metadata!);
    expect(trendMeta.bugfixCount).toBe(3); // 60 天窗口内 3 个 bugfix（与 bugfix_count 同源）
    expect(["shortening", "lengthening", "stable", "insufficient"]).toContain(trendMeta.trend);
