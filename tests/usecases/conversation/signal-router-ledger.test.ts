...
/** 消息种子（conv-1/turn-1/otter-1 基架）；#775 起提为文件级供多 describe 复用 */
function seedMsg(db: Database.Database, m: Partial<Message> & { id: string }): void {
...
describe("SignalRouter × 真实台账（S2 回放判据）", () => {
  ...
  it("R2 多獭稳态：墓碑覆盖的历史零误点（rbsg 126-invoke 免疫验证）", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      seedMsg(db, { id: `msg-old-${i}`, createdAt: `2026-09-01T0${i}:00:00Z` });
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// #775 S4a：routeDirectSignal 直投通道（scheduler/招聘入口换轨）
// ─────────────────────────────────────────────────────────────────────────────
describe("SignalRouter routeDirectSignal（#775 S4a：入口直投 + 闸门 + 记账）", () => {
  let db: Database.Database;
  let repo: SqliteDispatchAttemptRepo;
  let executeChain: ReturnType<typeof vi.fn>;
  let router: SignalRouter;

  function seedSystemMsg(id: string): void {
    // 行动类 system 消息（scheduler 锚点同款：completed + 带 tsp）——判据清零后对路由器可见
    seedMsg(db, { id, senderType: "system", senderId: "system", talkingStonePassedTo: ["otter-1"] });
  }
  ...
  beforeEach(() => {
    ...
    router = new SignalRouter({
      ...
      queryMessage: {
        ...
        // D4 busy 判定依赖：读真实库（.getLastMessageBySender → isOtterActive）
        getLastMessageBySender: vi.fn().mockImplementation((_convId: string, senderId: string) => {
          const row = db.prepare("SELECT * FROM messages WHERE sender_id = ? ORDER BY sequence_num DESC LIMIT 1").get(senderId) as { id: string; conversation_id: string; sender_type: string; sender_id: string; status: string; talking_stone_passed_to: string | null; signal_level: string | null; created_at: string } | undefined;
          if (!row) return null;
          return makeMsg({
            id: row.id, conversationId: row.conversation_id, status: row.status as Message["status"],
            senderType: row.sender_type as Message["senderType"], senderId: row.sender_id,
            talkingStonePassedTo: row.talking_stone_passed_to ? JSON.parse(row.talking_stone_passed_to) : [],
            signalLevel: row.signal_level, createdAt: row.created_at,
          });
      ...
      queryOtter: {
        getById: vi.fn().mockResolvedValue({ id: "otter-1", name: "大獭", type: "big", status: "active" }),
      } as unknown as QueryOtter,
      dispatchChainEngine: { executeChain } as unknown as DispatchChainEngine,
      invokeFn: vi.fn().mockResolvedValue({ messageId: "m-out" }),
      logger: createTestLogger(),
  ...
  it("D1 行动类 system 消息直投 → 点火 + 点火即记账（in_progress 非 pending）", async () => {
    seedSystemMsg("msg-d1");
    const action = await router.routeDirectSignal("conv-1", "msg-d1", "otter-1");
    expect(action).toBe("invoked");
    expect(executeChain.mock.calls).toHaveLength(1);
    const attempts = repo.listAttemptsForConversation("conv-1");
    expect(attempts).toHaveLength(1);
    expect(attempts[0].status).toBe("in_progress"); // 待链 settle 写终态
    expect(attempts[0].messageId).toBe("msg-d1");
  });

  it("D1b 直投链带 ledgerSource='router'（S2 标签失真修复：终态行可审计路由器点火）", async () => {
    seedSystemMsg("msg-d1b");
    await router.routeDirectSignal("conv-1", "msg-d1b", "otter-1");
    expect(executeChain.mock.calls).toHaveLength(1);
    const call = executeChain.mock.calls[0][0] as { ledgerSource?: string };
    expect(call.ledgerSource).toBe("router");
  });

  it("D2 消息缺失 → DirectChainGatedError（调用方记 skipped，不空转）", async () => {
    await expect(router.routeDirectSignal("conv-1", "msg-ghost", "otter-1"))
      .rejects.toThrowError(expect.objectContaining({ name: "DirectChainGatedError", gate: "skipped_no_signal" }));
    expect(executeChain).not.toHaveBeenCalled();
  });

  it("D3 用户停机中直投 → DirectChainGatedError(skipped_halted)，信号保留在台账", async () => {
    seedSystemMsg("msg-d3");
    router.markUserHalt("conv-1");
    await expect(router.routeDirectSignal("conv-1", "msg-d3", "otter-1"))
      .rejects.toThrowError(expect.objectContaining({ gate: "skipped_halted" }));
    expect(executeChain).not.toHaveBeenCalled();
    // 信号保留：无 attempt 行 = pending 仍在（恢复后补扫点燃）
    expect(repo.countPendingSignals("conv-1")).toBe(1);
  });

  it("D4 目标 busy → 入 busyQueue（queued_busy），内容经快照消化不丢", async () => {
    seedSystemMsg("msg-d4");
    // 预置 streaming 消息让 isOtterActive 为真（busy 判定）
    db.prepare(`INSERT INTO messages (id, conversation_id, sender_type, sender_id, status, sequence_num, turn_id, talking_stone_passed_to, created_at, completed_at)
      VALUES ('m-busy', 'conv-1', 'otter', 'otter-1', 'streaming', 2, 'turn-1', '[]', ?, NULL)`).run(new Date().toISOString());
    const action = await router.routeDirectSignal("conv-1", "msg-d4", "otter-1");
    expect(action).toBe("queued_busy");
    expect(executeChain).not.toHaveBeenCalled();
  });

  it("D5 直投后补扫不重复点火（幂等：点火即记账，路由器扫描跳过有账信号）", async () => {
    seedSystemMsg("msg-d5");
    await router.routeDirectSignal("conv-1", "msg-d5", "otter-1");
    await router.routePendingSignals("conv-1"); // 模拟 debounce 重扫
    expect(executeChain.mock.calls).toHaveLength(1); // 双跑被台账挡住
  });
});
