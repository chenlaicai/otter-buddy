...
describe("AssistantSessionManager", () => {
  ...
  it("收篇摘要时间戳为 Asia/Shanghai 本地时间，非 UTC 直出（A2 修复验证）", async () => {
    ...
    // mock 按真实仓库语义实现：limit:1（maybeRotate 阈值判断）返回旧时间触发轮换，
    // 无 options（writeDigest 全量拉取）返回固定锚定时间
    const ctx = makeManager({ lastEntryAgeHours: 0, rotationHours: 1 });
    const oldEntries = [
    ...
    ctx.deps.entryRepo.getEntries.mockImplementation(async (_id: string, options?: { entryType?: string; limit?: number }) => {
      if (options?.limit === 1) return [oldEntries[0]]; // maybeRotate 阈值判断（昨日时间，超 1h 阈值触发轮换）
